                ...
                <div class="overflow-x-scroll scrollbar-hide mb-4 relative px-0.5" style="overflow-y: hidden;">
                    <div class="flex snap-x snap-mandatory gap-4" style="width: max-content;">
                        @foreach ($events as $event)
                            <div class="cursor-pointer flex-none w-64 snap-center post"
                            ...
                            <div class="flex snap-x snap-mandatory gap-4" style="width: max-content;">
                                @foreach ($category->events as $event)
                                    <div class="cursor-pointer flex-none w-64 snap-center post"
                                        onclick="location.href='{{ route('reservations.detail', ['event' => $event->id]) }}'">
                                        <div
                                            class="bg-white border-1 border border-gray-200 rounded-lg overflow-hidden mb-4">
                                            @if (is_null($event->image))
                                                <img src="{{ asset('storage/No_Image.png') }}" alt=""
                                                    class="w-full h-40 object-cover">
                                            @else
                                            ...
                                            <div class="p-4">
                                                <h3 class="text-lg leading-6 font-bold text-gray-900">
                                                    {{ $event->title }}</h3>
                                                <h4 class="text-sm leading-6 text-gray-900">{{ $event->event_date }}
                                                </h4>
                                                <div class="flex justify-between items-center mt-4">
                                                    <span
                                                        class="text-2xl font-extrabold text-gray-900">{{ $event->formatted_price == 0 ? '無料' : $event->formatted_price . '円' }}</span>
