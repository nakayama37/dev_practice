                   ...
                   <x-error-message/>
                    ...
                    <form method="POST" action="{{ route('events.update', ['event' => $event->id ]) }}" enctype="multipart/form-data">
                        ...
                        <div class="mt-4">
                           ...
                           <select id="categories" name="categories[]" class="block w-full sm:w-2/3 bg-gray-200 py-2 px-3 text-gray-700 border border-gray-200 rounded focus:outline-none focus:bg-white" multiple>
                            @foreach($categories as $category)
                              <option name="category" value="{{ $category->id }}"  {{ $event->categories->contains($category->id) ? 'selected' : '' }} >{{ $category->name}} </option>
                            @endforeach
                          </select>
                        </div>
                        <div class="mt-4">
                        ...
                        <div class="md:flex justify-between items-end">
                          <div class="mt-4">
                            ...
                            <x-text-input id="max_people" class="block mt-1 w-full" type="number" name="max_people" value="{{ $event->max_people }}" required min="1"/>
                          </div>
                          <div class="mt-4">
                            <x-input-label for="price" value="価格（円）" />
                            <x-text-input id="price" class="block mt-1 w-full" type="number" name="price" value="{{ $event->price }}" min="0"/>
                          </div>
